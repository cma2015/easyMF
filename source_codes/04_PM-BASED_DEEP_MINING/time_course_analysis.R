...
TimeCourseAnalysis <- function(){
  ...
  pdf(options$pdf1)
  plot(Pattern[,1], pch = 20, type = "o", xaxt="n", xlab = "Stage", ylab = "PM coefficient", ylim = range(Pattern))
  axis(1,c(1:nrow(Pattern)),labels=Stage[rownames(Pattern),2])
  for(k in 2:ncol(Pattern)){
    points(Pattern[,k], pch = 20, col = k, type="o")
  }
  legend(x=1, y = max(Pattern), legend = c(paste0("Metagene ", 1:ncol(Pattern)))
       , col = 1:ncol(Pattern), border = NA
       , bty = "n", lty = c(rep(1, ncol(Pattern))), lwd = 2)
  dev.off()
  ...
  for(cl in 1:length(geneExp_sig_gene)){
    ...
    GSEA[[names(geneExp_sig_gene)[cl]]] <- list(result = result, ap = ap)
  }
  
  result <- NULL
  for(cl in 1:length(geneExp_sig_gene)){
    result <- rbind(result,cbind("Metagene"=paste0("Metagene",cl),GSEA[[cl]]$result))
