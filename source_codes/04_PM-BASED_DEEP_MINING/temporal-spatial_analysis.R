...
SpatialCourseAnalysis <- function(){
  ...
  PCCVal <- as.numeric(options$input5)
  PVal <- as.numeric(options$input6)
  library(biomaRt)
  mart <- biomaRt::useMart(biomart = "plants_mart", dataset =  options$input8, host = "plants.ensembl.org")
  GTOGO <- biomaRt::getBM(attributes = c("ensembl_gene_id", "go_id", "description","name_1006"), mart = mart)
  ...
  geneExp_sig_gene <- expressionMarkers(Amplitude = Amplitude, Pattern = Pattern, geneExp = gene_expression
                                        , cpu = cpu, PatternTC = TC, PCC = PCCVal, pvalue = PVal)
